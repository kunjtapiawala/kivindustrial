...
const CatalogMegaMenu = ({ onClose, activeCategoryId, onSelectCategory }: CatalogMegaMenuProps) => {
  ...
  return (
    <div className="absolute left-0 right-0 top-full z-40 hidden sm:block">
      <div className="mx-auto mt-3 flex max-w-6xl rounded-3xl border border-slate-200 bg-white text-slate-900 shadow-2xl">
        <aside className="w-72 overflow-y-auto border-r border-slate-200 bg-slate-50/70 p-6" style={{ maxHeight: "70vh" }}>
          <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.35em] text-slate-500">
            Categories
            <button
              type="button"
              onClick={onClose}
              className="rounded-md px-2 py-1 text-slate-500 transition hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
            >
              Close
            </button>
          </div>
              ...
              return (
                <li key={category.id}>
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 ${isActive ? "bg-white text-slate-900 shadow-sm" : "text-slate-600 hover:bg-white/70 hover:text-slate-900"}`}
                  >
                    <span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold ${isActive ? "bg-slate-900 text-white" : "bg-slate-200 text-slate-600"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-8" style={{ maxHeight: "70vh" }}>
          <header className="border-b border-slate-200 pb-4">
            <h3 className="text-2xl font-semibold text-slate-900">{activeCategory.name}</h3>
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs font-medium text-slate-500">
              {activeCategory.highlights.map((highlight) => (
                <span key={highlight} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1">
                  {highlight}
                </span>
              ))}
            </div>
          ...
          <div className="mt-8 flex flex-wrap gap-3 text-sm">
            <Link
              ...
              onClick={onClose}
              className="rounded-full bg-slate-900 px-5 py-2 font-semibold text-white transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
            >
            ...
            <Link
              href="/catalog"
              onClick={onClose}
              className="rounded-full border border-slate-300 px-5 py-2 font-semibold text-slate-700 transition hover:border-slate-400 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
            >
